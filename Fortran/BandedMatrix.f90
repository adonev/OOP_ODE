module BandedMatrix
    ...
    type, extends(LinOp), public :: BandMat
        ...
        real(wp), dimension(:,:), allocatable :: A, A_p_aI, invA, expA ! Matrix A, A+alpha*Id, (A+alpha*Id)^(-1), and exp(A*t)
...
subroutine InitBandMat(this,matrix,t,alpha,dt,tol) ! Initialize (set values, factorize, etc.)
   ...
   if(all(this%n_bands==[0,0])) then ! Diagonal matrix
       if(.not.(size(matrix,2)==1)) write(*,*) "WARNING: matrix should be of size (n,1) for diagonal matrices", size(matrix)
       ...
       if(.not.(size(matrix,2)==3)) write(*,*) "WARNING: matrix should be of size (n,3) for diagonal matrices", size(matrix)
       ...
       if(.false.) then ! Introduce a BUG on purpose
           if(.not.(size(matrix,2)==n)) write(*,*) "WARNING: matrix should be of size (n,n) for dense matrices", size(matrix) 
